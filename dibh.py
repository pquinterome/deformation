...
y_train = y_train.cache().map(preprocess_image_train, num_parallel_calls=AUTOTUNE).batch(3)

y_test = tf.data.Dataset.from_tensor_slices(y_testy)
y_test = y_test.cache().map(preprocess_image_train, num_parallel_calls=AUTOTUNE).batch(3)
...
dataset_train = dataset_train.cache().map(lambda item:(tf.py_function(read_npy_file, [item], [tf.float64,]))).batch(3)
dataset_test = tf.data.Dataset.from_tensor_slices(test)
dataset_test = dataset_test.cache().map(lambda item:(tf.py_function(read_npy_file, [item], [tf.float64,]))).batch(3)
