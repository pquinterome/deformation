...
x = Conv3D(filters=164, kernel_size=(8,8), activation='relu', padding='same')(i)
x = MaxPool3D(pool_size=(8,8))(x)
x = Conv3D(filters=64, kernel_size=(6,6), activation='relu', padding='same')(x)
x = MaxPool3D(pool_size=(6,6))(x)
x = Flatten()(x)
...
ax.errorbar(data=new_dfw, y='Predicted Values', x='True Values', yerr=new_dfw['Standard Deviation'], fmt='none', capsize=0,  color='gray')
plt.xlim(-1.1,1.1)
plt.savefig('outputs/MRI_model_performance.png', bbox_inches='tight')
print('MRI_MODEL')
print('y_test', y_test)
print('Predictions', predy)
print('MAE', me)
